		...
		if(pPart)
		{
			if(IVehiclePart *pParentPart = pPart->GetParent())
			{
				...
				if(ICharacterInstance *characterInst = pVehicle->GetEntity()->GetCharacter(pParentPart->GetSlot()))
				{
					...
					{
						ICharacterModelSkeleton* pICharacterModelSkeleton = characterInst->GetICharacterModel()->GetICharacterModelSkeleton();
						int16 joint = pICharacterModelSkeleton->GetJointIDByName(pPart->GetName());
						...
						if (pl)
						{
							...
							else
							{
								...
								int boneId = pCharacter ? pCharacter->GetICharacterModel()->GetICharacterModelSkeleton()->GetJointIDByName("Spine03") : 7;
    ...
    if (IEntity *entity = player->GetEntity())
    {
      ...
      if (ICharacterInstance *character = entity->GetCharacter(0))
      {
        ...
        for(uint32 i=0; i<numAnimsLayer; i++)
        {
          ...
          {
            float time = m_CurrentTime; //pos; //cry_fmod(aimRad / ANIM_ANGLE_RANGE, 1.0f);
