...
#if defined(PS3) || defined(ORBIS)	// FIXME: Using PS3 inputs for ORBIS
...
public:
	...
	virtual void GetConfiguration(SFlowNodeConfig& config)
	{
		...
		static const SOutputPortConfig out_config[] =
		{
			...
			OutputPortConfig_AnyType("XBoxController", _HELP("Outputs the signal from Check input if the current control scheme is XBox controller")),
			OutputPortConfig_AnyType("PS3Controller", _HELP("Outputs the signal from Check input if the current control scheme is PS3 controller")),
			{0}
		};
	...
	virtual void ProcessEvent(EFlowEvent event, SActivationInfo* pActInfo)
	{
		if (event == eFE_Activate)
		{
			...
			case eControlScheme_KeyboardMouse:
				...
				break;
			case eControlScheme_XBoxController:
				ActivateOutput(pActInfo, XBoxController, GetPortAny(pActInfo, Get));
				break;
			case eControlScheme_PS3Controller:
				ActivateOutput(pActInfo, PS3Controller, GetPortAny(pActInfo, Get));
				break;
...
public:
	...
	virtual bool OnControlSchemeChanged(const EControlScheme controlScheme)
	{
		...
		case eControlScheme_KeyboardMouse:
			...
			break;
		case eControlScheme_XBoxController:
			ActivateOutput(&m_actInfo, XBoxController, true);
			break;
		case eControlScheme_PS3Controller:
			ActivateOutput(&m_actInfo, PS3Controller, true);
			break;
		}
		return false;
	}
